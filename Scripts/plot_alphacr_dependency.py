...
x0 = 0.49
DATA_PATH = 'C:/_Repositories/KaschenkoEquation/Tracer/Results/x0=0.49'
CSV_FILE = 'x0=0.49_analytical.csv'
AFTER_TANGENT = True
AFTER_TANGENT_FILE = 'x0=0.49_analytical_after_tangent.csv'
...
def visualise_alphas(gs, aus, acs, gfs, afs):
    ...
    dec_gs = list(map(decimal.Decimal, list(map(str, gs))))
    prec_gs = [d.as_tuple().exponent for d in dec_gs[1:]]
    l = prec_gs.index(-6)
    plt.scatter(gs[l], aus[l], color='black', s=14, zorder=5)
    if AFTER_TANGENT:
    ...
    plt.axhline(y=0.0, linewidth=2, color='darkgrey', zorder=2)
    plt.axvline(x=0.0, linewidth=2, color='darkgrey', zorder=2)
    #x1,x2,y1,y2 = plt.axis()
    #plt.axis((x1,x2,-45,10))
